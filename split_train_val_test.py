...
class DevelopDataset:
    ...
    note - the only folders needed are jpg, T2, tumors by annotator
    if you re-process, delete jpg_tumors, model_dev_indvPIRADS, tumors

    '''
    random.seed(9001)

    def __init__(self):
        self.basePATH=r'C:\Users\sanfordt\Desktop\PIRADS_dataset\revision_analysis_numpy'
        self.jpg=os.path.join(self.basePATH,'jpg')
        self.jpg_tumors=os.path.join(self.basePATH,'jpg_tumors')
        self.tumors = os.path.join(self.basePATH,'tumors')
        self.model_dev=os.path.join(self.basePATH,'model_dev_indvPIRADS')
    ...
    def split_train_val_test(self,val_fraction=0.2,test_fraction=0.1,balance=False,clip=False):
        ...
        patients=[patient for patient in os.listdir(self.jpg_tumors)]
    ...
    def find_sample(self,pts):
        ...
        pts_out=[]
        for pat in pts:
            pts_out+=[pat+'_'+tumor for tumor in os.listdir(os.path.join(self.jpg_tumors, pat))]
        return pts_out
        ...
        for patient in os.listdir(os.path.join(self.jpg_tumors)):
            ...
            for tumor_name in os.listdir(os.path.join(self.jpg_tumors,patient)):
                if not os.path.exists(os.path.join(self.tumors, patient+'_'+tumor_name)):
                    ...
                    copy_tree(os.path.join(self.jpg_tumors,patient,tumor_name),os.path.join(self.tumors, patient+'_'+tumor_name))
    ...
    def comb_slices_by_tumor(self,mid=True):
        ...
        for patient_dir in os.listdir(os.path.join(self.jpg)):
            ...
            for key in patient_dict.keys():
                ...
                jpg_name=slices[0].split('_')[3]+'_'+slices[0].split('_')[4]+'_'+slices[0].split('_')[5]\
...
if __name__=='__main__':
    ...
    c.anon_recurs(os.path.join(c.model_dev))
